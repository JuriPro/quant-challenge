        ...
        try:
            ...
            while dt.datetime.now() <= time_of_completion:
                try:
                    ...
                    log.error(f"Error fetching or writing snapshot: {e}")
            ...
            log.error(f"Failed to collect L2 data: {e}")
            return f"Failed to collect L2 data: {e}"
...
class PreprocessBacktestData:
    async def handle(self, command: PreprocessRawL2Data):
        ...
        try:
            ...
            with open(file_path, 'r') as file:
                for snapshot in file:
                    try:
                        ...
                        log.error(f"Error processing snapshot: {e}")
            ...
            log.error(f"Failed to preprocess data: {e}")
...
class PerformBacktest:
    async def handle(self, command: PerformBacktestCmd):
        ...
        try:
            ...
            bt = Backtest(backtest_df, OrderBookStrategy, cash=100000, commission=.002, exclusive_orders=True,
            ...
            log.error(f"Failed to perform backtest: {e}")
            return f"Failed to perform backtest: {e}"
